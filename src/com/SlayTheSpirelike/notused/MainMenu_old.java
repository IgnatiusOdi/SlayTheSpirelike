...
import com.SlayTheSpirelike.FontLoader;

import java.awt.*;

public class MainMenu_old extends State{

    private Body_old body;
    private Button newGame;

    public MainMenu_old(Body_old body) {
        this.body = body;
            ...
            public void tick() {
                if(body.getMouse().getX() >= x && body.getMouse().getX() <= (x + widht) && body.getMouse().getY() >= y && body.getMouse().getY() < (y + height))
                {
                    ...
                    if(body.getMouse().isClicked())
                    {
                        ...
                        State.setCurrent(new CharSelect_old(body));
